...
export const createMongoSchemaChangeHandler =
  (mongoService: MongoService) =>
  ({ schemas }: SchemaChangeHandler) => {
    if (!mongoService)
      throw new Error(
...
export const createLocalSchemaChangeHandler =
  () =>
  ({ schemas }: SchemaChangeHandler) => {
    const init = () => {
      eventBus.subscribe({
...
export const createTenantCache = ({
  ...
  createSchemaChangeHandler: ({ schemas }: SchemaChangeHandler) => {
    init: () => void;
  };
}) => {
  ...
  createSchemaChangeHandler({ schemas }).init();
